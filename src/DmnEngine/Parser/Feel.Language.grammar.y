...
// 1
expression:		  textualExpression					{ Debug("expression/text");	$$ = $1; }
				| boxedExpression					{ Debug("expression/box");	$$ = $1; }
				;

// 2
textualExpression:	  functionDefinition				{ Debug("textualExpression/funDef");	$$ = $1; }
...
// 4
arithmeticExpression: addition					{ Debug("arithmeticExpression/add"); $$ = $1; }
...
// 5
simpleExpression:	  arithmeticExpression		{ Debug("simpleExpression/arith");	$$ = $1; }
					| simpleValue				{ Debug("simpleExpression/simVal");	$$ = $1; }
					;

// 6
simpleExpressions:	  simpleExpression								{ Debug("simpleExpressions");	$$ = new ExpressionList(); $$.Add($1); }
					| simpleExpressions COMMA simpleExpression		{ Debug("simpleExpressions/,"); $$ = $1;				   $$.Add($3); }
					;

// 7, 8
simplePositiveUnaryTest:  endpoint												{ Debug("simplePositiveUnaryTest");		$$ = new Comparison(ComparisonOperator.Equal, new InputValue(), $1); }
...
// 8
simplePositiveUnaryTestOp:	  C_LT		{ $$ = ComparisonOperator.LessThan;			  }
...
// 9, 10
intervalStart:	  closedIntervalStart	{ $$ = false; }
				| SP_OPEN				{ $$ = true;  }
				;
...
// 11, 12
intervalEnd:  closedIntervalEnd	{ $$ = false; }
			| SP_CLOSE			{ $$ = true;  }
			;
...
// 13
simplePositiveUnaryTests:	  simplePositiveUnaryTest									{ Debug("simplePositiveUnaryTests");	$$ = $1;			 }
...
// 16
positiveUnaryTests:	  positiveUnaryTest								{ Debug("positiveUnaryTests");	 $$ = $1;			  }
					| positiveUnaryTests COMMA positiveUnaryTest	{ Debug("positiveUnaryTests/,"); $$ = new Or($1, $3); }
					;

// 17
unaryTests:   positiveUnaryTests							{ Debug("unaryTests");		$$ = $1;					   }
...
// 19
simpleValue:  simpleLiteral				{ Debug("simpleValue/lit");	$$ = $1; }
...
// 33
literal:  simpleLiteral	{ Debug("literal/simLit");	$$ = $1; }
		| NULL		    { Debug("literal/null");	$$ = $1; }
		;

// 34
simpleLiteral:    numericLiteral	{ Debug("simpleLiteral/num");	$$ = $1; }
...
// 37-39
numericLiteral:			   NUMBER	{ Debug("numericLiteral");     $$ = new NumericLiteral( $1); }
...
// 41
parameters:   P_OPEN namedParameters P_CLOSE	{ Debug("parameters/named");	$$ = $1; }
...
// 49
quantorOp:	  SOME		{ $$ = QuantorOperator.Some;  }
			| EVERY		{ $$ = QuantorOperator.Every; }
			;

// 46,48
inParts:  inPart					{ Debug("inParts");   $$ = new InPartList($1); }
...
// 51
comparisonOp: C_NE		{ $$ = ComparisonOperator.NotEqual;			  }
...
// 55
boxedExpression:  list					{ Debug("boxedExpression/list");	$$ = $1; }
...
// 57
functionDefinition:   FUNCTION P_OPEN formalParameterList P_CLOSE expression
					| FUNCTION P_OPEN formalParameterList P_CLOSE EXTERNAL expression
					;

// 57
formalParameterList:  formalParameter								{ Debug("formatParameterList/,"); $$ = new FormalParameterList($1); }
...
// 62
dateTimeLiteral:  DATE P_OPEN stringLiteral P_CLOSE				{ $$ = DateTimeLiteral.ParseDate(((StringLiteral) $3).Value);		 }
